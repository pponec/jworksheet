 ...
 * @author Pavel Ponec
 */
public class SingleUjoTabModel extends UjoTableModel<KeyRow> implements Iterable<KeyRow> {

    /** Property row */
    public static final KeyRow ROWS = null;

    /**
     * Creates a new instance of SingleUjoTabModel
     */
    public SingleUjoTabModel(Ujo content) {
        this(content, UjoManager.getInstance().readKeys(KeyRow.class).toArray() );
    }
    ...
    public boolean isCellEditable(int rowIndex, Key column) {
        ...
        =  column==KeyRow.P_VALUE
        || column==KeyRow.P_TEXT
        ;
        return result;
    }
    ...
    public Object getValueAt(int rowIndex, Key column) {
        ...
        ?  getCoder().encodeValue(getRow(rowIndex).getKey().getDefault(), false)
    ...
    public Iterator<KeyRow> iterator() {
